...
const STATS = [
  { icon: '⚽', value: '12,847', label: 'Predicciones realizadas' },
  { icon: '👥', value: '1,203', label: 'Usuarios activos' },
  { icon: '🏆', value: '89', label: 'Ligas creadas' },
  { icon: '🗓️', value: '342', label: 'Jornadas disputadas' },
]
...
const LEADERBOARD = [
  { pos: 1, name: 'CarlosGol',     pts: 1847, change: '+3', streak: 8,  medal: '🥇', color: 'border-amber-500/40 shadow-amber-500/10' },
  { pos: 2, name: 'LaPelota10',   pts: 1792, change: '+1', streak: 5,  medal: '🥈', color: 'border-slate-400/30 shadow-slate-400/5'  },
  { pos: 3, name: 'TacticaFC',    pts: 1731, change: '—',  streak: 3,  medal: '🥉', color: 'border-amber-700/30 shadow-amber-700/5'  },
  { pos: 4, name: 'GolazoMex',    pts: 1690, change: '+2', streak: 4,  medal: '',   color: '' },
  { pos: 5, name: 'ElPronostico', pts: 1644, change: '-1', streak: 2,  medal: '',   color: '' },
]
    ...
    <main className="min-h-screen bg-[#0B1020] text-slate-50 overflow-x-hidden">
      ...
      >
        <div className="container mx-auto px-5 py-4 flex items-center justify-between max-w-7xl">
          ...
          <Link href="/" className="flex items-center gap-2.5 group">
            <div className="w-9 h-9 rounded-xl bg-green-500 flex items-center justify-center shadow-lg shadow-green-500/40 group-hover:shadow-green-500/70 group-hover:scale-105 transition-all duration-300">
        ...
        <div className="container mx-auto px-5 max-w-7xl">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {STATS.map((s, i) => (
              <div
                key={i}
                className="group bg-[#131A2E] border border-white/5 hover:border-green-500/20 rounded-2xl p-6 text-center transition-all duration-300 hover:shadow-xl hover:shadow-green-500/5 hover:-translate-y-1"
              >
        ...
        <div className="container mx-auto px-5 max-w-7xl">
          ...
          <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {STEPS.map((s, i) => (
              <div
                key={i}
                className={`group relative bg-[#131A2E] border ${s.border} rounded-3xl p-8 hover:shadow-2xl ${s.glow} transition-all duration-300 hover:-translate-y-2`}
              >
                <div className={`text-xs font-black mb-4 font-mono ${s.badge}`}>{s.num}</div>
        ...
        <div className="container mx-auto px-5 max-w-7xl relative z-10">
          ...
          <div className="grid md:grid-cols-3 gap-5">
            {MATCHES.map((m, i) => (
              <div
                key={i}
                className="group bg-[#131A2E] border border-white/5 hover:border-white/10 rounded-3xl p-6 transition-all duration-300 hover:shadow-2xl hover:shadow-black/50 hover:-translate-y-2"
              >
                {/* Top bar */}
                <div className="flex items-center justify-between mb-6">
                  <span className="text-xs text-slate-500 font-medium truncate pr-2">{m.tournament}</span>
                  {m.status === 'live' ? (
                    <div className="flex items-center gap-1.5 bg-red-500/20 border border-red-500/30 rounded-full px-2.5 py-1 flex-shrink-0">
                      <span className="w-1.5 h-1.5 rounded-full bg-red-400 animate-pulse" />
                      <span className="text-red-400 text-xs font-black">LIVE</span>
                    </div>
                  ) : (
                    <span className="text-xs text-slate-500 bg-white/5 px-2.5 py-1 rounded-full flex-shrink-0">{m.time}</span>
                  )}
                </div>
                ...
                <div className="flex items-center justify-between mb-6">
                  <div className="text-center">
                    ...
                    <p className="text-sm font-bold text-white">{m.home.name}</p>
                  </div>
                  <div className="text-center px-2">
                    {m.status === 'live' ? (
                      <>
                        <div className="text-2xl font-black text-white">{m.home.score} – {m.away.score}</div>
                        <p className="text-xs text-red-400 font-bold mt-0.5">{m.time}</p>
                      </>
                    ) : (
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-sm font-bold text-white">{m.away.name}</p>
                  </div>
                </div>
                ...
                <div className="border-t border-white/5 pt-4">
                  <div className="flex items-center justify-between text-xs text-slate-500 mb-2">
                    <span>Picks populares</span>
                    <span>{m.picks}% predicen local</span>
                  </div>
                  ...
                  <Link
                    href="/register"
                    className="w-full block text-center bg-white/5 hover:bg-green-500/15 hover:text-green-400 text-slate-400 text-xs font-bold py-2.5 rounded-xl transition-all duration-200"
                  >
                    Hacer predicción →
                  </Link>
                </div>
            ...
            <div className="bg-[#131A2E] border border-white/5 rounded-2xl overflow-hidden mb-6">
              {LEADERBOARD.slice(3).map((p) => (
                ...
                >
                  ...
                  <span className="text-xs text-green-400 font-bold">{p.streak}🔥</span>
                  <span className={`text-xs font-bold w-8 text-right ${p.change.startsWith('+') ? 'text-green-400' : p.change === '—' ? 'text-slate-500' : 'text-red-400'}`}>
                    {p.change}
                  </span>
                  <span className="text-sm font-black text-slate-300 w-12 text-right">{p.pts}</span>
                </div>
              ))}
            </div>
      ...
      <section className="py-24 bg-[#0d1424]">
        <div className="container mx-auto px-5 max-w-7xl">
          <div className="text-center mb-16">
            ...
            <h2 className="text-4xl md:text-5xl font-black text-white mb-4">Funcionalidades</h2>
          </div>
      ...
      <section className="py-28 relative overflow-hidden">
        ...
        <div className="container mx-auto px-5 max-w-7xl text-center relative z-10">
          ...
          <Link
            href="/register"
            className="inline-block bg-green-500 hover:bg-green-400 text-black font-black text-lg py-5 px-12 rounded-2xl transition-all duration-200 shadow-2xl shadow-green-500/30 hover:shadow-green-500/55 hover:-translate-y-1"
          >
            Crear cuenta gratis →
          </Link>
        </div>
